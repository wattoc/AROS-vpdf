...
char *ConvertString(char *str, const char *from_charset, const char *to_charset, int length)
{
  char *result = NULL;
  ...
  else
  {
    ...
    if(dstCodeset != NULL)
    {
      ...
      if(fromIsUTF8 == TRUE)
      {
        dstText = CodesetsUTF8ToStr(CSA_Source,      (long unsigned int)str,
                                    CSA_DestCodeset, (long unsigned int)dstCodeset,
                                    CSA_DestLenPtr,  (long unsigned int)&dstLen,
                                    CSA_ErrPtr,      (long unsigned int)&errPtr,
                                    TAG_DONE);
      }
      else
      {
        ...
        if(srcCodeset != NULL)
        {
          dstText = CodesetsConvertStr(CSA_Source,        (long unsigned int)str,
                                       CSA_SourceCodeset, (long unsigned int)srcCodeset,
                                       CSA_DestCodeset,   (long unsigned int)dstCodeset,
                                       CSA_DestLenPtr,    (long unsigned int)&dstLen,
                                       CSA_ErrPtr,        (long unsigned int)&errPtr,
                                       TAG_DONE);
          ...
          D(kprintf("Unknown from charset: %s\n", from_charset));
      }
      ...
      if(dstText != NULL && dstLen != 0 && errPtr == 0)
      {
        ...
        if((buf = (char*)malloc(dstLen+1)) != NULL)
        {
      ...
      {
        if(errPtr != 0)
          D(kprintf("Invalid characters found: %s\n", errPtr));
      ...
      D(kprintf("Unknown dest charset: %s\n", to_charset));
  }

  return strndup(result,length);
;
}
...
{
#ifdef __AROS__	
	return ConvertString((char*)string,"UTF-16","ISO-8859-1", length);
#else
...
{
#if defined(__AROS__)
	return ConvertString((char*)string,"UTF-32","ISO-8859-1", length);
#else
	char *buff = (char*)calloc(1, length + 1);
	if(buff != NULL)
...
#ifdef __AROS__
// ToUCS4
//original mos function: WCHAR ToUCS4(char asciichar, CONST struct KeyMap *keyMap );
//os4 replace: 
static uint32* unicode_map;
//struct Locale *locale;

WCHAR ToUCS4(uint32 asciichar, CONST struct KeyMap *keyMap)
{
//	locale = OpenLocale(NULL);
//	unicode_map = (uint32*)ObtainCharsetInfo(DFCS_NUMBER, locale->loc_CodeSet, DFCS_MAPTABLE);  
	
    if (unicode_map && asciichar <= 255 && unicode_map[asciichar] <= 65535)
        return unicode_map[asciichar];
...
int pdfSearch(void *_ctx, int *page, char *phrase, int direction, double *x1, double *y1, double *x2, double *y2)
{
	...
	else if(ctx->search.phrase == NULL || 0 != strcmp(ctx->search.phrase, phrase) || ctx->search.page != *page)
	{
		...
		D(kprintf("Find phrase: [%ls]\n",phraseUCS4));
		TextOutputDev *text_dev = new TextOutputDev(NULL, gTrue, 0, gFalse, gFalse);
		D(kprintf("got textoutputdev\n"));
